...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: virtualApptRow } = await supabase
      .from("virtual_appointments")
      .select("id")
      .eq("stripe_checkout_session_id", sessionId)
      .maybeSingle();

    if (virtualApptRow?.id) {
      try {
        await handleVirtualAppointmentPaid(
          supabase,
          session,
          `evt_confirm_${sessionId}`,
        );
      } catch (e) {
        console.error("checkout/confirm virtual appointment:", e instanceof Error ? e.message : e);
        captureException(e, { route: "checkout_confirm", type: "virtual_appointment" });
        return NextResponse.json({ error: "No se pudo confirmar la cita virtual." }, { status: 500 });
      }
      return NextResponse.json({ ok: true, type: "virtual_appointment" });
    }
    ...
    try {
      ...
      captureException(mailErr, { route: "checkout_confirm", step: "smtp_client" });
      console.error(
    ...
    captureException(error, { route: "checkout_confirm" });
