...
export default function ClientDashboardContent({
  ...
  nextVirtual: { starts_at: string; meet_link: string | null; reason: string } | null;
}) {
  const router = useRouter();
  const locale = useLocale();
  const tV = useTranslations("VirtualAppointment");
  ...
  return (
    <main className="min-h-screen bg-[#FFFAF7] py-8 px-4">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mb-6 rounded-2xl border border-rosa-100 bg-white p-4 sm:p-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <h2 className="text-sm font-medium text-warm-dark mb-1">{tV("clientNextTitle")}</h2>
              {nextVirtual ? (
                <>
                  <p className="text-base text-warm-gray-600">
                    {new Date(nextVirtual.starts_at).toLocaleString(locale === "en" ? "en-GB" : "es-ES", {
                      dateStyle: "full",
                      timeStyle: "short",
                      timeZone: "Europe/Madrid",
                    })}
                  </p>
                  {nextVirtual.reason?.trim() ? (
                    <p className="mt-2 text-sm text-warm-gray-600">
                      <span className="font-medium text-warm-dark">{tV("clientReasonLabel")}:</span>{" "}
                      {nextVirtual.reason.trim()}
                    </p>
                  ) : null}
                  {nextVirtual.meet_link ? (
                    <a
                      href={nextVirtual.meet_link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-3 inline-flex items-center justify-center rounded-full bg-warm-dark px-4 py-2 text-sm font-medium text-white hover:bg-warm-gray-500 transition-colors"
                    >
                      {tV("clientMeet")}
                    </a>
                  ) : (
                    <p className="mt-2 text-xs text-warm-gray-400">{tV("clientNoMeet")}</p>
                  )}
                </>
              ) : (
                <p className="text-sm text-warm-gray-500">{tV("clientNoUpcoming")}</p>
              )}
            </div>
            <Link
              href="/cita-virtual"
              className="shrink-0 inline-flex items-center justify-center rounded-full border border-rosa-200 bg-rosa-50/80 px-4 py-2.5 text-sm font-medium text-rosa-500 hover:bg-rosa-100 transition-colors text-center"
            >
              {tV("clientNextCta")}
            </Link>
          </div>
