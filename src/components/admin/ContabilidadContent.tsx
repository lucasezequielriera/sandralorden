                  ...
                  <tr key={inv.id} className="group hover:bg-warm-white transition-all duration-200">
                    ...
                    <td className="px-3 py-3.5 text-center">
                      <button
                        ...
                        aria-label={`Cambiar estado de factura: ${inv.concept} (${statusLabels[inv.status] ?? inv.status})`}
                    ...
                    <td className="pr-4 py-3.5">
                      <button onClick={() => handleDelete(inv.id)}
                        className="text-warm-gray-200 hover:text-[#8C4A47] transition-colors cursor-pointer opacity-0 group-hover:opacity-100" aria-label={`Eliminar factura: ${inv.concept}`}>
