...
export default function SupabaseAuthRecoveryBridge() {
  ...
  useEffect(() => {
    ...
    // Evita carrera con páginas de login que consumen el hash.
    if (path.includes("/login") && (search.includes("nueva=1") || search.includes("reset=1"))) {
      return;
    }
    ...
    void (async () => {
      ...
      if (session?.user && isRecovery) {
        if (path.includes("/admin")) {
          router.replace({ pathname: "/admin/login", query: { reset: "1" } });
        } else {
